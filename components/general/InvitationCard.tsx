  ...
  return (
    <div className="bg-card shadow-md rounded-md p-5 border border-gray-200">
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() =>
            dispatch({ type: "userDescription/open", payload: jobseeker })
          }
          className="bg-blue-400 hover:bg-blue-500 transition-[background] text-white rounded-md text-sm px-3 py-1"
        >
          Detail
        </button>
        <p
          className={`${
            item.status === "on review"
              ...
              ? "bg-green-600"
              : "bg-red-500"
          } text-white text-sm capitalize rounded-md px-3 py-1`}
        >
          {item.status}
        </p>
      </div>
