  ...
  return (
    <>
      <div className="bg-card rounded-md border border-gray-200 shadow-md p-5">
        ...
        <div className="mt-6">
          <p className="text-sm bg-gray-200 w-fit rounded-full px-4 py-1 text-gray-600 truncate">
            {data.job.organization?.industryType} Industry
          </p>
        </div>
        <div className="mt-6 flex items-center gap-3">
          <FaUsers className="text-xl text-gray-600" />
          <p className="text-sm">
            &plusmn; {data.job.organization?.totalEmployee} employees
          </p>
        </div>
        <div className="flex items-center justify-between mt-5">
          <button
            onClick={() => setOpenJobDetailModal(true)}
            className="bg-blue-500 hover:bg-blue-600 transition-[background] rounded-md text-white px-4 py-2 text-sm"
          >
            Detail
          </button>
          {data.status === "on review" ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-green-600 hover:bg-green-700 transition-[background] rounded-md text-white px-3 text-lg py-2"
              >
                <MdCheck />
              </button>
              <button
                ...
                className="bg-red-500 hover:bg-red-600 transition-[background] rounded-md text-white px-3 py-2 text-lg"
              >
                <AiOutlineClose />
              </button>
            </div>
            ...
            <p
              className={`${
                data.status === "accepted" ? "bg-green-600" : "bg-red-500"
              } rounded-md capitalize text-sm text-white px-3 py-2`}
            >
              {data.status}
            </p>
          )}
